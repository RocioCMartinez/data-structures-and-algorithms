...
function transformToLis(obj){
  ...
  for (let key in obj) {
    let listItem = `<li>${key}: ${obj[key]}</li>`;
    list.push(listItem);
  }
  return list;
}
...
const count = (target, input) => {
  // Solution code here...
  let number = input.reduce((acc, currentInput) => {
    let times = currentInput.filter((num) => num === target).length;
    return acc + times;
  }, 0);
...
const totalSum = (input) => {
  // Solution code here...
  let total = input.reduce((acc, currentInput) => {
    let sum = currentInput.reduce((inputSum, num) => inputSum + num, 0);
    return acc + sum;
  }, 0);
  return total;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  let newArr = input.map((innerArr) => {
    return innerArr.filter((element) => typeof element === 'number' && element % 5 === 0).map((element) => Math.pow(2, element));
  });
  return newArr;
};
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  let mnfChrs = data.filter(character => character.gender === 'male' || character.gender === 'female').map(character => character.name);
  let combinedNames = mnfChrs.join(' and ');
...
let findShortest = (data) => {
  // Solution code here...
  let shortest = data.reduce((short, character) => {
    if(!short || parseInt(character.height) < parseInt(short.height)) {
      return character;
    }
    return short;
  });
