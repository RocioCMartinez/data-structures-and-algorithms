...
const maxInArray = (arr) => {
  // Solution code here...
  return arr.reduce((acc, intVal) => acc > intVal ? acc : intVal);
};
...
const getCourseKeys = (obj) => {
  // Solution code here...
  let newArr = Object.keys(courseInfo);
  return newArr;
};
...
const getHouses = (arr) => {
  ...
  for (let element of arr){
    if (element.house && !houses.includes(element.house));
...
const hasChildrenValues = (arr, character) => {
  // Solution code here...
  let match = arr.find(obj => obj.name === character);

  if (!match || !match.children){
    return false;
  }
  return Object.values(match.children).length > 0;
};
